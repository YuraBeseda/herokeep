import { type Entity, EntitySchema } from '@hk/protocol';
 ...
 * overlay win). `cite`/`note` are documentation only and never reach the entity.
 */
 ...
 * deep-merged in (see `deepMerge`). The patched entity is re-validated with `EntitySchema` — an
 * invalid result throws with the Zod issue. An overlay whose `id` matches no entity throws
 * (`Unmatched overlay: <id>`), so a stale correction can never silently rot. Pure: neither
 * `entities` nor its elements are mutated — a fresh array of fresh entities is returned.
 */
