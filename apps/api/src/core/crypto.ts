 ...
 * (`globalThis.crypto.subtle`/`getRandomValues`) only — no `node:crypto` — so this file runs
 * unchanged on both the Node adapter (>= 20) and Cloudflare Workers, and stays inside the
 * `src/core/**` boundary rule (no `node:*`/`cloudflare:*` imports).
 */
...
export async function hmacSha256Hex(key: string, msg: string): Promise<string> {
  ...
  const signature = await globalThis.crypto.subtle.sign('HMAC', cryptoKey, toBytes(msg));
