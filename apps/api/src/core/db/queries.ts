...
import { campaigns, characters, memberships, recoveryCodes, sessions, usageDaily, users } from './schema.ts';
...
/** Inserts a new campaign index row (`POST /api/campaigns`, Task 4). Throws on a `join_code`
 * UNIQUE-constraint violation — the route retries with a freshly generated code
 * (`./join-code.ts`'s `generateJoinCode`) rather than this function handling the retry, mirroring
 * `insertUser`'s "throws, caller decides" shape. Does not touch `memberships` — the route inserts
 * the DM's own membership row separately (design ruling 3's dual-write: D1 rows first, event
 * append second; `campaigns` and the DM's `memberships` row are both part of that same D1-first
 * write). */
export async function createCampaign(db: Db, row: NewCampaign): Promise<Campaign> {
  const [created] = await db.insert(campaigns).values(row).returning();
  if (!created) throw new Error('createCampaign: insert returned no row');
  return created;
}

/** Looks up a campaign by id (route ownership/membership checks, Task 4). */
export async function findCampaignById(db: Db, id: string): Promise<Campaign | undefined> {
  const [row] = await db.select().from(campaigns).where(eq(campaigns.id, id)).limit(1);
  return row;
}

/** Looks up a campaign by its join code (`POST /api/campaigns/join`, Task 4). `join_code` is
 * UNIQUE at the DB level (schema.ts), so this can only ever match zero or one row. */
export async function findCampaignByJoinCode(db: Db, joinCode: string): Promise<Campaign | undefined> {
  const [row] = await db.select().from(campaigns).where(eq(campaigns.joinCode, joinCode)).limit(1);
  return row;
}

/** Every campaign a user can see (`GET /api/campaigns`, Task 4): campaigns they DM plus campaigns
 * they're a member of, deduplicated, with no repeats even for a DM whose own `memberships` row
 * also exists (design ruling 1 keeps that row in sync but this function doesn't rely on it being
 * present).
 *
 * Implementation: two queries (dm-of via `campaigns.dmId`, member-of via a `memberships` join),
 * merged and de-duplicated by id in a `Map` on the JS side — not a single SQL `UNION` query. This
 * keeps the query portable across the sync better-sqlite3 driver and the async D1 driver without
 * needing raw-SQL `UNION` support from Drizzle's sqlite-core query builder, at the cost of two
 * round-trips instead of one; campaign counts per user are small (no invented per-user cap, doc-08
 * has none) so this is not a hot path worth the extra complexity. */
export async function listCampaignsForUser(db: Db, userId: string): Promise<Campaign[]> {
  const dmOf = await db.select().from(campaigns).where(eq(campaigns.dmId, userId));
  const memberOf = await db
    .select({ campaign: campaigns })
    .from(memberships)
    .innerJoin(campaigns, eq(memberships.campaignId, campaigns.id))
    .where(eq(memberships.userId, userId));

  const byId = new Map<string, Campaign>();
  for (const row of dmOf) byId.set(row.id, row);
  for (const { campaign } of memberOf) byId.set(campaign.id, campaign);
  return [...byId.values()];
}

/** Sets a campaign's `join_code` to a freshly generated one (`POST /api/campaigns/:id/rotate-code`,
 * Task 4, DM-only). `newCode` is generated by the caller (`./join-code.ts`'s `generateJoinCode`,
 * retried by the caller on a UNIQUE violation) — this function just writes it. */
export async function rotateJoinCode(db: Db, campaignId: string, newCode: string): Promise<void> {
  await db.update(campaigns).set({ joinCode: newCode }).where(eq(campaigns.id, campaignId));
}

/** Writes a campaign index row's `bytes_used` directly from its stream's own meta — the campaign
 * analogue of `updateCharacterUsage` (see that function's doc comment for the full staleness
 * contract this mirrors): the daily maintenance job's per-stream sync (Task 9), never a per-event
 * writer. Deliberately narrow (only this one column) for the same reason
 * `updateCharacterUsage` is narrow: a nightly sync must not perturb `updated_at`-based ordering. */
export async function updateCampaignUsage(db: Db, campaignId: string, bytesUsed: number): Promise<void> {
  await db.update(campaigns).set({ bytesUsed }).where(eq(campaigns.id, campaignId));
}

/** Every campaign index row across every DM — the daily maintenance job's campaign-usage-sync
 * source (Task 9, the campaign analogue of `listAllCharacters`) and any future admin-export
 * coverage of campaigns. Unlike `listCampaignsForUser`, not scoped to one user. */
export async function listAllCampaigns(db: Db): Promise<Campaign[]> {
  return db.select().from(campaigns);
}

/** Removes a campaign's index row (hard delete/archive-purge). Does not touch its `memberships`
 * rows (FK-referenced by `campaign_id`, `schema.ts`) or the campaign stream's own storage —
 * callers that need those clean up separately, mirroring `deleteCharacterIndexRow`'s scope. */
export async function deleteCampaignIndexRow(db: Db, id: string): Promise<void> {
  await db.delete(campaigns).where(eq(campaigns.id, id));
}

// --- memberships ---------------------------------------------------------------------------

/** Inserts a membership row (campaign create's DM row, or `POST /api/campaigns/join`'s new
 * member row, Task 4). Throws (composite-PK constraint violation) if `(campaignId, userId)`
 * already exists — a user can only join a given campaign once; `findMembership` is the caller's
 * pre-check when a friendlier error than a raw constraint violation is wanted. */
export async function insertMembership(db: Db, row: NewMembership): Promise<Membership> {
  const [created] = await db.insert(memberships).values(row).returning();
  if (!created) throw new Error('insertMembership: insert returned no row');
  return created;
}

/** Removes one membership row (`DELETE /api/campaigns/:id/members/:userId`, Task 4, DM-only;
 * also a member's own "leave campaign"). Returns whether a row was actually deleted, the same
 * "caller maps false to 404/no-op" shape as `deleteSessionById`. */
export async function removeMembership(db: Db, campaignId: string, userId: string): Promise<boolean> {
  const result = await db
    .delete(memberships)
    .where(and(eq(memberships.campaignId, campaignId), eq(memberships.userId, userId)))
    .returning({ campaignId: memberships.campaignId });
  return result.length > 0;
}

/** Looks up one membership row by its composite key — the join-route's "already a member" check
 * and the gateway/permission code's role lookup (design ruling 1). */
export async function findMembership(db: Db, campaignId: string, userId: string): Promise<Membership | undefined> {
  const [row] = await db
    .select()
    .from(memberships)
    .where(and(eq(memberships.campaignId, campaignId), eq(memberships.userId, userId)))
...
/** Every membership row for a campaign — the party/member-list view (Task 4/5). */
export async function listMembers(db: Db, campaignId: string): Promise<Membership[]> {
  return db.select().from(memberships).where(eq(memberships.campaignId, campaignId));
}

/** Count of a campaign's membership rows — the 12-member quota check (Global Constraints:
 * "20 MB events / 12 members / 6 non-core packs × 5 MB") the join route enforces before inserting
 * a new membership. */
export async function countMembers(db: Db, campaignId: string): Promise<number> {
  const [row] = await db
    .select({ count: sql<number>`count(*)` })
    .from(memberships)
